...
class HiddenLayersCountException(ValueError):
    """
    Exception raised for count hidden layers
    """
...
def count_epochs(
    len_input: int, len_output: int, len_dataset: int, stop_criterion_decibels: float, neurons: List[int]
) -> int:
    """
...
def num_neurons(
    ...
    """
    len_input - number of input parameters
    ...
    raise HiddenLayersCountException(
        f"Number hidden layers is not correct: min = {MIN_NUM_HIDDEN_LAYERS}, max = {MAX_NUM_HIDDEN_LAYERS}"
    )


def find_hyperparametres(
    len_dataset: int, len_output: int, len_input: int, num_hidden_layers: int = 2
) -> Tuple[Dict[BorderAssessment, Dict[str, Union[List[int], int]]], Tuple[float, float]]:
    ...
    dict_hyper: Dict[BorderAssessment, Dict[str, Union[List[int], int]]] = {}
